...
class BaselineTrain(nn.Module):
    ...
    def forward_meta_val(self, x):
        ...
        for i in range(self.params.val_n_way):
            fake_support = torch.index_select(z_query,dim=0,index=indices[i]).view(1,4,-1)
        ...
        z_proto = r_f_support.contiguous().view(self.params.val_n_way, self.params.n_shot+4, -1 ).mean(1)
        #z_proto = z_support.contiguous().view(self.params.val_n_way, self.params.n_shot, -1).mean(1)
