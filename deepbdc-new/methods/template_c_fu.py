...
class BaselineTrain(nn.Module):
    ...
    def forward_meta_val(self, x):
        ...
        indices_add_fu = np.zeros((self.params.val_n_way,c))
        #print(indices.shape,indices_add_fu.shape)
        c_add_fu = 0
        for i in range(5):
            for j in range(20):
                if y_label[indices[i][j]] == i:
                    ...
                    indices_add_fu[i][c_add_fu] = indices[i][j]
                    c_add_fu = c_add_fu + 1
                    #rint(c_add_fu,c)
                    if c_add_fu == c:
                        c_add_fu = 0
        ...
        for i in range(self.params.val_n_way):
            fake_support = torch.index_select(z_query,dim=0,index=indices_add_fu[i]).view(1,c,-1)
        ...
        z_proto = r_f_support.contiguous().view(self.params.val_n_way, self.params.n_shot+c, -1 ).mean(1)
        #z_proto = z_support.contiguous().view(self.params.val_n_way, self.params.n_shot, -1).mean(1)
