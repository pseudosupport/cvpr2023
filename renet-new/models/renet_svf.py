...
from models.resnet_svf import ResNet
...
class RENet(nn.Module):
    ...
    def cca(self, spt, qry):
        ...
        if self.args.shot > 1:
            if flag == 1:
                spt_attended = spt_attended.view(num_qry, int(way/5), self.args.way, *spt_attended.shape[2:])
                qry_attended = qry_attended.view(num_qry, int(way/5), self.args.way, *qry_attended.shape[2:])
            elif flag == 0:
