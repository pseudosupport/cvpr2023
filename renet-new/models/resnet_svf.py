...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        #w = self.conv1.weight
        #U, S, V = torch.svd(w)
        #self.conv1.weight = Parameter(V)
        out = self.conv1(x)
        #print(x.shape,out.shape,w.shape,U.shape,S.shape,V.shape)
        #raw_input()
        #out = out.mul(S)
        #self.conv1_1.weight = Parameter(U)
        print('1:',x.shape,out.shape,self.conv1.weight.shape)
        #out = self.conv1_1(out)
        out = self.bn1(out)
        out = self.relu(out)

        out = self.conv2(out)
        print('2:',out.shape,self.conv2.weight.shape)
        out = self.bn2(out)
        out = self.relu(out)

        out = self.conv3(out)
        print('3:',out.shape,self.conv3.weight.shape)
        out = self.bn3(out)
