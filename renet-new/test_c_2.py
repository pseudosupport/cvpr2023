...
def evaluate(epoch, model, loader, args=None, set='val'):
    ...
    with torch.no_grad():
        for i, (data, labels) in enumerate(tqdm_gen, 1):
            ...
            for i in range(5):
                fake_support = torch.index_select(data_query,dim=0,index=indices[i]).view(1,8,640,5,5)
            ...
            data_shot_new = r_f_support.contiguous().view(args.way, args.shot+8, 640,5,5).mean(1).view(args.way,640,5,5)
