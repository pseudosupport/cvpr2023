...
from test_c_2 import test_main, evaluate
...
def train(epoch, model, loader, optimizer, args=None):
    ...
    for i, ((data, train_labels), (data_aux, train_labels_aux)) in enumerate(zip(tqdm_gen, train_loader_aux), 1):
        ...
        values,indices = torch.topk(dists_1_t,12,largest=True)
        for i in range(5):
            fake_support = torch.index_select(data_query,dim=0,index=indices[i]).view(1,12,640,5,5)
            r_support = data_shot[i].view(1,1,640,5,5)
        ...
        data_shot_new = r_f_support.contiguous().view(args.way, args.shot+12, 640,5,5 ).mean(1).view(args.way,640,5,5)
        logits_new,_ = model((data_shot_new.unsqueeze(0).repeat(args.num_gpu, 1, 1, 1, 1), data_query))
        epi_loss = F.cross_entropy(logits_new, label)
...
if __name__ == '__main__':
    ...
    print('test/acc:', test_acc, 'test/confidence_interval:', test_ci)
    print('time_all',all_time)
