...
def train(epoch, model, loader, optimizer, args=None):
    ...
    for i, ((data, train_labels), (data_aux, train_labels_aux)) in enumerate(zip(tqdm_gen, train_loader_aux), 1):
        ...
        for i in range(5):
            fake_support = torch.index_select(data_query,dim=0,index=indices[i]).view(1,4,640,5,5)
        ...
        data_shot_new = r_f_support.contiguous().view(args.way, args.shot+4, 640,5,5 ).mean(1).view(args.way,640,5,5)
